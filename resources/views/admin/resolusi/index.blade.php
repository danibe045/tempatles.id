<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <p class="text-[10px] font-black text-rose-500 uppercase tracking-[0.25em] mb-1">Pusat Bantuan</p>
                <h2 class="font-black text-2xl text-slate-900 leading-tight tracking-tight">
                    Resolusi Komplain & Sengketa
                </h2>
                <p class="text-sm text-slate-400 font-medium mt-0.5">Tengahi masalah antara murid dan tutor untuk menentukan nasib dana Escrow.</p>
            </div>
            
            <form method="GET" action="{{ route('admin.resolusi') }}" class="flex flex-wrap md:flex-nowrap gap-2 w-full md:w-auto">
                <div class="relative">
                    <select name="status" onchange="this.form.submit()"
                        ...
                        <option value="sedang_dimediasi" {{ request('status') == 'sedang_dimediasi' ? 'selected' : '' }}>Sedang Dimediasi</option>
                        <option value="selesai_refund" {{ request('status') == 'selesai_refund' ? 'selected' : '' }}>Selesai (Refund)</option>
                        <option value="selesai_tolak" {{ request('status') == 'selesai_tolak' ? 'selected' : '' }}>Selesai (Ditolak)</option>
                    </select>
                ...
                <div class="relative">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama pelapor..." 
                        class="pl-9 pr-4 py-2.5 bg-slate-100 border-transparent rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-200 focus:bg-white focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all w-full md:w-52 placeholder:font-medium placeholder:text-slate-400 shadow-sm">
                    <div class="pointer-events-none absolute inset-y-0 left-2.5 flex items-center">
    ...
    <div class="py-8">
        <div class="max-w-[1400px] mx-auto px-6 md:px-12 space-y-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                ...
                {{-- Menunggu Review (Rose/Merah Gradient) --}}
                <div class="bg-gradient-to-br from-rose-500 to-rose-700 rounded-2xl shadow-lg shadow-rose-500/20 p-6 relative overflow-hidden group border border-rose-400/50">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-[10px] font-black text-rose-100 uppercase tracking-[0.2em]">Laporan Baru</p>
                        <p class="text-4xl font-black text-white mt-1 leading-none">{{ $countMenunggu }}</p>
                    </div>
                </div>

                {{-- Sedang Dimediasi (Orange Gradient) --}}
                <div class="bg-gradient-to-br from-orange-400 to-orange-600 rounded-2xl shadow-lg shadow-orange-500/20 p-6 relative overflow-hidden group border border-orange-400/50">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-[10px] font-black text-orange-100 uppercase tracking-[0.2em]">Sedang Mediasi</p>
                        <p class="text-4xl font-black text-white mt-1 leading-none">{{ $countMediasi }}</p>
                    </div>
                </div>

                {{-- Kasus Selesai (Emerald Gradient) --}}
                <div class="bg-gradient-to-br from-emerald-400 to-emerald-600 rounded-2xl shadow-lg shadow-emerald-500/20 p-6 relative overflow-hidden group border border-emerald-400/50">
                    ...
                    <div class="relative z-10">
                        ...
                        <p class="text-[10px] font-black text-emerald-100 uppercase tracking-[0.2em]">Kasus Selesai</p>
            ...
            <div class="bg-white shadow-sm rounded-2xl overflow-hidden border border-gray-100">
                <div class="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-white">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-black text-slate-800 text-sm uppercase tracking-widest">Daftar Tiket Komplain</h3>
                    </div>
                </div>
                    ...
                    <table class="w-full text-sm text-left">
                        <thead>
                            <tr class="bg-slate-50/50 text-slate-400 uppercase text-[9px] tracking-[0.18em] font-black border-b border-gray-100">
                                <th class="px-6 py-4">Pelapor & Info Tiket</th>
                                <th class="px-6 py-4">Detail Pesanan</th>
                                <th class="px-6 py-4">Subjek Komplain</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-9 h-9 rounded-xl bg-rose-50 border border-rose-100 text-rose-600 flex items-center justify-center font-black text-sm uppercase shadow-sm">
                                            {{ substr($komplain->pelapor->name ?? 'U', 0, 1) }}
                                        </div>
                                        <div>
                                            <p class="font-bold text-slate-800 text-xs">{{ $komplain->pelapor->name ?? 'User Terhapus' }}</p>
                                            <p class="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">{{ $komplain->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                </td>
                                ...
                                {{-- Kolom 2: Order Ref --}}
                                <td class="px-6 py-4">
                                    <p class="font-black text-blue-950 text-xs">#ORD-{{ str_pad($komplain->order_id, 4, '0', STR_PAD_LEFT) }}</p>
                                    <p class="text-[10px] text-slate-500 font-medium mt-1 truncate max-w-[150px]">Tutor: {{ $komplain->order->tutor->name ?? '-' }}</p>
                                </td>

                                {{-- Kolom 3: Jenis Komplain --}}
                                <td class="px-6 py-4">
                                    <span class="inline-block bg-slate-100 text-slate-700 text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider mb-1">
                                        {{ $komplain->jenis_komplain }}
                                    </span>
                                    <p class="text-xs font-medium text-slate-500 truncate max-w-[200px]">{{ $komplain->deskripsi }}</p>
                                </td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    @php
                                        $statusClasses = [
                                            'menunggu_review'  => 'bg-rose-50 text-rose-600 border-rose-200',
                                            'sedang_dimediasi' => 'bg-orange-50 text-orange-600 border-orange-200',
                                            'selesai_refund'   => 'bg-emerald-50 text-emerald-700 border-emerald-200',
                                            'selesai_tolak'    => 'bg-slate-100 text-slate-600 border-slate-300',
                                        ];
                                        $badgeClass = $statusClasses[$komplain->status] ?? 'bg-gray-50 text-gray-600 border-gray-200';
                                    @endphp
                                    <span class="inline-flex items-center justify-center px-2.5 py-1.5 rounded border {{ $badgeClass }} text-[8px] font-black uppercase tracking-wider shadow-sm">
                                        {{ str_replace('_', ' ', $komplain->status) }}
                                    </span>
                                </td>

                                {{-- Kolom 5: Aksi --}}
                                <td class="px-6 py-4 text-right">
                                    <a href="#" class="inline-flex items-center gap-1.5 bg-white hover:bg-slate-900 text-slate-600 hover:text-white font-black py-2 px-3 rounded-xl transition-all duration-200 text-[9px] uppercase tracking-widest border border-slate-200 shadow-sm">
                                        Investigasi
                                    </a>
                                </td>
                            </tr>
                            ...
                            <tr>
                                <td colspan="5" class="text-center py-20 bg-slate-50/30">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <p class="text-sm font-bold text-slate-500 mt-2">Tidak ada komplain aktif</p>
                                        <p class="text-xs text-slate-400">Semua kelas berjalan dengan lancar tanpa sengketa.</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
